...
function RecipeManagerModal({ product, inventoryItems, submitting, onClose, onSubmit }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}><section className="employee-modal settings-modal recipe-modal" role="dialog" aria-modal="true" aria-labelledby="manage-recipe-title"><header><div><span className="eyebrow">Strict inventory recipe</span><h2 id="manage-recipe-title">{product.name}</h2><p>Enter the ingredient quantity consumed by one order of this menu item.</p></div><button className="icon-button" type="button" onClick={onClose} aria-label="Close"><X size={19} /></button></header><form onSubmit={(event) => { event.preventDefault(); onSubmit(components) }}><div className="recipe-list">{inventoryItems.map((item) => <label key={item.id}><span><strong>{item.name}</strong><small>Used per one order · unit: {item.unit}{item.secondary_unit ? ` (also shown in ${item.secondary_unit})` : ''}</small></span><input type="number" min="0" max="999999" step={item.unit === 'pcs' ? '1' : '0.001'} value={quantities[item.id] || ''} onChange={(event) => setQuantities({ ...quantities, [item.id]: event.target.value })} placeholder={`0 ${item.unit}`} /></label>)}</div>{!inventoryItems.length && <div className="settings-empty"><PackageCheck size={28} /><strong>No inventory ingredients</strong><span>Add inventory items before configuring a product recipe.</span></div>}<div className="recipe-rule"><ShieldCheck size={18} /><span>Checkout is blocked when this recipe is missing or when any ingredient has insufficient stock. Piece-based recipes require whole pieces.</span></div><div className="employee-modal__actions"><button className="secondary-button" type="button" onClick={onClose}>Cancel</button><button className="primary-button" type="submit" disabled={submitting || !components.length}>{submitting ? 'Saving...' : 'Save strict recipe'}</button></div></form></section></div>
}
...
export function SettingsPage({ accessToken, profile }) {
  ...
  return <div className="page settings-page">
    ...
    {recipeProduct && <RecipeManagerModal product={recipeProduct} inventoryItems={data?.inventoryItems || []} submitting={submitting} onClose={() => setRecipeProduct(null)} onSubmit={saveRecipe} />}
  </div>
}
