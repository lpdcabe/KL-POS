...
function RecipeManagerModal({ product, inventoryItems, submitting, onClose, onSubmit, onUsePieces }) {
  const existing = Object.fromEntries((product.product_recipes || []).map((entry) => [entry.inventory_item_id, String(entry.quantity_required)]))
  const [quantities, setQuantities] = useState(existing)
  const components = inventoryItems
    .map((item) => ({ inventoryItemId: item.id, quantityRequired: Number(quantities[item.id] || 0) }))
    .filter((component) => component.quantityRequired > 0)

  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}><section className="employee-modal settings-modal recipe-modal" role="dialog" aria-modal="true" aria-labelledby="manage-recipe-title"><header><div><span className="eyebrow">Recipe deduction</span><h2 id="manage-recipe-title">{product.name}</h2><p>For one customer order, how much stock should be deducted?</p></div><button className="icon-button" type="button" onClick={onClose} aria-label="Close"><X size={19} /></button></header><form onSubmit={(event) => { event.preventDefault(); onSubmit(components) }}><div className="recipe-list">{inventoryItems.map((item) => <label key={item.id}><span><strong>{item.name}</strong><small>{item.unit === 'pcs' ? 'Counted as individual pieces' : `Currently tracked by ${item.unit}`}</small>{item.unit === 'kg' && !item.secondary_unit && <button className="recipe-unit-switch" type="button" onClick={() => onUsePieces(item.id)}>Use pieces instead</button>}</span><div className="unit-input"><input type="number" min="0" max="999999" step={item.unit === 'pcs' ? '1' : '0.001'} value={quantities[item.id] || ''} onChange={(event) => setQuantities({ ...quantities, [item.id]: event.target.value })} placeholder="0" /><b>{item.unit}</b></div></label>)}</div>{!inventoryItems.length && <div className="settings-empty"><PackageCheck size={28} /><strong>No inventory ingredients</strong><span>Add inventory items before configuring a product recipe.</span></div>}<div className="recipe-rule"><ShieldCheck size={18} /><span>Example: for “Chicken Wings – 4 pcs,” enter <strong>4</strong> beside Chicken Wings after changing it to piece tracking.</span></div><div className="employee-modal__actions"><button className="secondary-button" type="button" onClick={onClose}>Cancel</button><button className="primary-button" type="submit" disabled={submitting || !components.length}>{submitting ? 'Saving...' : 'Save recipe'}</button></div></form></section></div>
}
...
export function SettingsPage({ accessToken, profile }) {
  ...
  async function saveRecipe(components) {
    setSubmitting(true)
    try {
      const result = await apiRequest(`/api/settings/products/${recipeProduct.id}/recipe`, { accessToken, method: 'PATCH', body: JSON.stringify({ components }) })
      setData({ ...data, products: data.products.map((product) => product.id === recipeProduct.id ? { ...product, product_recipes: result.recipe } : product) })
      setRecipeProduct(null)
      setSuccess('Strict inventory recipe saved.')
      setError('')
    } catch (requestError) {
      setError(requestError.message)
    } finally {
  ...
  return <div className="page settings-page">
    ...
    {recipeProduct && <RecipeManagerModal product={recipeProduct} inventoryItems={data?.inventoryItems || []} submitting={submitting} onClose={() => setRecipeProduct(null)} onSubmit={saveRecipe} onUsePieces={(itemId) => navigate(`/inventory?setupPieces=${itemId}`)} />}
  </div>
}
